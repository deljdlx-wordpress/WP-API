...
class Client
{
    ...
    public function __construct($apiRootURL, $user, $password)
    {
        $this->user = $user;
    ...
    {
        return json_decode(file_get_contents($this->apiRootURL . '/categories/?search='.urlencode($search)));
    }

    public function searchMedia($search)
    {
        return json_decode(file_get_contents($this->apiRootURL . '/media/?search='.urlencode($search)));
    }

    public function searchPost($search)
    {
        return json_decode(file_get_contents($this->apiRootURL . '/posts/?search='.urlencode($search)));
    }
    ...
    public function updatePostCategories($postId, $categories)
    {
        ...
        $url = $this->apiRootURL . '/posts/'.$postId;
        $ch = curl_init();
    ...
    public function createCategory($name, $description = '', $slug = null)
    {
        ...
        $url = $this->apiRootURL . '/categories/';
        $ch = curl_init();
        curl_setopt( $ch, CURLOPT_URL, $url );
        curl_setopt( $ch, CURLOPT_POST, 1 );
        curl_setopt( $ch, CURLOPT_POSTFIELDS, $data);
